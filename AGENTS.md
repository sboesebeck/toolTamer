...
- `tui/` is the Python/Textual half: the interactive TUI plus the helper tools (`tui/cleanup_deps.py`, `tui/fix_taps.py`, `tui/warm_deps.py`, `tui/ttignore.py`, `tui/secrets.py`). Core logic in `tui/core/` (including the shared ignore engine, `tui/core/ignore.py`, and the age secret layer, `tui/core/secrets.py` + `tui/core/secret_ops.py`), screens in `tui/screens/`, widgets in `tui/widgets/`.
...
- **Secrets keep `tui/core/config` out of the encrypt/decrypt path.** `tui/secrets.py` imports `config`/`secret_ops` lazily inside the commands that need the hierarchy; the `encrypt`/`decrypt` subcommands Bash calls during a sync must run with stdlib + `age` only (no `pathspec`/venv on a fresh host). Do not hoist those imports to the top.
- **Scope membership is derived, never stored as truth.** `secrets/scopes/<scope>.members` is only a cache so `ensure_scope` can skip a needless re-wrap (age ciphertext is non-deterministic; a repeated `join` must not dirty the repo). A scope is a config name; members are the registered machines whose include chain names it.
